...
static constexpr u_int8_t MPU9250_WHOAMI_DEFAULT_VALUE{0x71};
static constexpr u_int8_t MPU9255_WHOAMI_DEFAULT_VALUE{0x73};
...
class MPU9250_
{
    static constexpr u_int8_t MPU9250_DEFAULT_ADDRESS{0x68}; // Device address when ADO = 0
    static constexpr u_int8_t AK8963_ADDRESS{0x0C};          //  Address of magnetometer
    static constexpr u_int8_t AK8963_WHOAMI_DEFAULT_VALUE{0x48};
    u_int8_t MPU9250_ADDRESS{MPU9250_DEFAULT_ADDRESS};

    // settingsk
    MPU9250Setting setting;
    static constexpr u_int8_t MAG_MODE{0x06}; // 0x02 for 8 Hz, 0x06 for 100 Hz continuous magnetometer data read
...
public:
    ...
    {
        u_int8_t c = read_byte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);
        if (b_verbose)
        {
    ...
    {
        u_int8_t c = read_byte(AK8963_ADDRESS, AK8963_WHO_AM_I);
        if (b_verbose)
        {
    ...
    float getAcc(const u_int8_t i) const { return (i < 3) ? a[i] : 0.f; }
    float getGyro(const u_int8_t i) const { return (i < 3) ? g[i] : 0.f; }
    float getMag(const u_int8_t i) const { return (i < 3) ? m[i] : 0.f; }
    float getLinearAcc(const u_int8_t i) const { return (i < 3) ? lin_acc[i] : 0.f; }
    ...
    float getAccBias(const u_int8_t i) const { return (i < 3) ? acc_bias[i] : 0.f; }
    float getGyroBias(const u_int8_t i) const { return (i < 3) ? gyro_bias[i] : 0.f; }
    float getMagBias(const u_int8_t i) const { return (i < 3) ? mag_bias[i] : 0.f; }
    float getMagScale(const u_int8_t i) const { return (i < 3) ? mag_scale[i] : 0.f; }
...
private:
    void initMPU9250()
    {
        ...
        u_int8_t mpu_config = (u_int8_t)setting.gyro_dlpf_cfg;
        write_byte(MPU9250_ADDRESS, MPU_CONFIG, mpu_config);

        // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
        u_int8_t sample_rate = (u_int8_t)setting.fifo_sample_rate;
        ...
        u_int8_t c = read_byte(MPU9250_ADDRESS, GYRO_CONFIG); // get current GYRO_CONFIG register value
        c = c & ~0xE0;                                        // Clear self-test bits [7:5]
        c = c & ~0x03;                                        // Clear Fchoice bits [1:0]
        c = c & ~0x18;                                        // Clear GYRO_FS_SEL bits [4:3]
        c = c | (u_int8_t(setting.gyro_fs_sel) << 3);         // Set full scale range for the gyro
        c = c | (u_int8_t(setting.gyro_fchoice) & 0x03);      // Set Fchoice for the gyro
        ...
        c = c | (u_int8_t(setting.accel_fs_sel) << 3); // Set full scale range for the accelerometer
        ...
        c = c | ((u_int8_t(setting.accel_fchoice) & 0x01) << 3); // Set accel_fchoice_b to 1
        c = c | u_int8_t(setting.accel_dlpf_cfg);                // Set accelerometer rate to 1 kHz and bandwidth to 41 Hz
    ...
    void initAK8963()
    {
        ...
        u_int8_t raw_data[3];                          // x/y/z gyro calibration data stored here
        ...
        write_byte(AK8963_ADDRESS, AK8963_CNTL, (u_int8_t)setting.mag_output_bits << 4 | MAG_MODE); // Set magnetometer data resolution and sample ODR
        delay(10);
    ...
    {
        u_int8_t raw_data[14];                                       // x/y/z accel register data stored here
    ...
    {
        u_int8_t raw_data[7]; // x/y/z gyro register data, ST2 register stored here, must read ST2 at end of data acquisition
        if (read_byte(AK8963_ADDRESS, AK8963_ST1) & 0x01)
        {                                                               // wait for magnetometer data ready bit to be set
            ...
            u_int8_t c = raw_data[6];                                   // End data read by reading ST2 register
            if (!(c & 0x08))
    ...
    {
        u_int8_t raw_data[2];                                     // x/y/z gyro register data stored here
    ...
    void collect_acc_gyro_data_to(float *a_bias, float *g_bias)
    {
        ...
        u_int16_t fifo_count = ((u_int16_t)data[0] << 8) | data[1];
        u_int16_t packet_count = fifo_count / 12; // How many sets of full gyro and accelerometer data for averaging

        for (u_int16_t ii = 0; ii < packet_count; ii++)
        {
    ...
    void collect_mag_data_to(float *m_bias, float *m_scale)
    {
        ...
        for (u_int16_t ii = 0; ii < sample_count; ii++)
        {
    ...
    {
        u_int8_t raw_data[6] = {0, 0, 0, 0, 0, 0};
    ...
    */
    ...
    u_int8_t read_byte(u_int8_t address, u_int8_t subAddress) {
        u_int8_t data = 0;                         // `data` will store the register data
        wire->beginTransmission(address);         // Initialize the Tx buffer
        wire->write(subAddress);                  // Put slave register address in Tx buffer
